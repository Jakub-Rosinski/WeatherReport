...
mock_meteo_data = {"location": "Wrocław", "latitude": 51.1, "longitude": 17.039999,
                   "generationtime_ms": 0.3739595413208008,
...
@pytest.fixture
def instance():
    return AnalyzeWeatherData(mock_meteo_data, 15.0, 0.1)
...
def test_get_hourly_data(instance):
    time, temperature, rain = instance.get_hourly_data()
    assert len(time) == len(temperature) == len(rain)


def test_get_hourly_units_for_temp(instance):
    unit = instance.get_hourly_units("temperature_2m")
    assert unit == "°C"


def test_get_hourly_units_for_rainfall(instance):
    unit = instance.get_hourly_units("rain")
    assert unit == "mm"


def test_analyze(instance, loop: asyncio.AbstractEventLoop):
    res = loop.run_until_complete(instance.analyze())
    assert res is True
